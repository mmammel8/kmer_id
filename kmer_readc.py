...
jobs_name = "jobs3c"
save_target = 0 #4555 #Zea mays
foldername = "chloroplast"
fastadir = "/mnt/dmb/Mark_backup/plastid/"
...
process = Popen(['./kmerreadc', '-name', foldername, '-fadir', fastadir, '-jname', jobs_name, '-target', str(save_target)])
...
for i in range(num_targs):
    ...
    if rowmax[i] > 0.000: #only output non-zero results
        ...
        for j in range(num_cols):
            k = order_col[j]
            output += ',' + str(m[i,k]) + ',' + str(u[i]) + ',' + str(b[i,k]) 
        output += "\n"
        out_file.write(output)
out_file.close()
